	...
	if(req.isAuthenticated()) {  //can't allow edit if not logged in
			Campground.findById(req.params.id, function(err, campgroundfound) {
				if (err) {
					...
					if(campgroundfound.author.id.equals(req.user._id) || req.user.admin ) { //check readme.md for equal
						next();
					}else {
						res.redirect("back");
					};				
	...
	if(req.isAuthenticated()) {  //can't allow edit if not logged in
			Comments.findById(req.params.comment_id, function(err, commentfound) {
				if (err) {
					...
					if(commentfound.author.id.equals(req.user._id) || req.user.admin ) { //check readme.md for equal
						next();
					}else {
						res.redirect("back");
					};				
